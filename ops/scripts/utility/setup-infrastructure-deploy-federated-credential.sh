...
provision_identity() {
  ...
  if [[ "$GITHUB_ENVIRONMENT" == "infrastructure-deploy-main" ]]; then
    SECRET_VAR_NAME="AZ_INFRA_DEPLOY_MAIN_CLIENT_ID" # pragma: allowlist secret
  else
    SECRET_VAR_NAME="AZ_INFRA_DEPLOY_BRANCH_CLIENT_ID" # pragma: allowlist secret
  fi
