...
#          are used by the "Stand Alone E2E Test Runs" workflow (via
#          reusable-e2e.yml) to run end-to-end tests against the deployed
#          application.
#
...
#
# NOTE: The "Continuous Deployment" workflow also invokes reusable-e2e.yml. If
#       that workflow calls it with the same environments (e2e-main, e2e-branch),
#       additional federated credentials for the "Continuous Deployment" caller
#       will need to be added to these app registrations. Confirm caller workflow
#       names in reusable-e2e.yml before running if CD also calls it directly.
#
# Prerequisites:
...
GITHUB_WORKFLOW="Stand Alone E2E Test Runs"
TARGET="${TARGET:-all}"
...
case "$TARGET" in
  main)
    provision_identity "cams-e2e-main-oidc" "gha-e2e-main" "e2e-main"
    ;;
  branch)
    provision_identity "cams-e2e-branch-oidc" "gha-e2e-branch" "e2e-branch"
    ;;
  all)
    provision_identity "cams-e2e-main-oidc" "gha-e2e-main" "e2e-main"
    provision_identity "cams-e2e-branch-oidc" "gha-e2e-branch" "e2e-branch"
