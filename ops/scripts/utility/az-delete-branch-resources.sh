...
if [[ "${netExists}" == "true" && "${vnetIntegrationFailed}" == "true" ]]; then
    ...
    (
        ...
        if [[ "${unmanage_action}" != "deleteResources" ]]; then
            ...
            # azure-deploy-app-shared-setup.sh — three copies total. Can't
            # share it across bash/bicep without a codegen step, so keep all
            # three in lockstep by hand: if one changes, this vnet-link
            # lookup silently stops matching and falls to the "nothing to
            # delete" branch, leaking the link.
