...
# Usage:        ./az-func-slot-deploy-ustp.sh -h --src ./path/build.zip -g resourceGroupName -n functionappName --networkRg networkRgName --vnet vnet --subnet subnetName --slotName slotName
#
# Exitcodes
# ==========
# 0   No error
...
if [[ -n ${app_settings} ]]; then
    echo "Set Application Settings for ${app_name} ${slot_name}"
    # shellcheck disable=SC2086 # REASON: Adds unwanted quotes after --settings
    az functionapp config appsettings set -g "${app_rg}" -n "${app_name}" --settings ${app_settings} ${decorated_kv_settings} --slot ${slot_name} --query "[].name" --output tsv
fi
