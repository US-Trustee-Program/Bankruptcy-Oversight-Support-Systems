...
if [ -n "$restriction_names" ]; then
    while IFS= read -r rule_name; do
        ...
        az webapp config access-restriction remove -g "${app_rg}" -n "${app_name}" --slot "${slot_name}" --rule-name "${rule_name}" --scm-site true || echo "Failed to remove SCM rule: ${rule_name}"
    done <<< "$restriction_names"
fi

# Also remove main site restrictions
echo "Saving current main site access restrictions..."
saved_main_restrictions=$(az webapp config access-restriction show -g "${app_rg}" -n "${app_name}" --slot "${slot_name}" --query "ipSecurityRestrictions" -o json)
...
if [ -n "$main_restriction_names" ]; then
    while IFS= read -r rule_name; do
        ...
        az webapp config access-restriction remove -g "${app_rg}" -n "${app_name}" --slot "${slot_name}" --rule-name "${rule_name}" || echo "Failed to remove main site rule: ${rule_name}"
