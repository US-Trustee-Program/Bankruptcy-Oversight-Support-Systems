...
| ADMIN-NOTIFICATION-EMAIL       | Key Vault Secret       | Yes              | Optional. Email notified when an ACS email delivery-failure alert fires. Fetched from `kv-ustp-cams`/`kv-ustp-cams-dev` by `reusable-deploy.yml` (see "Fetch config from Key Vault" step), not a GitHub secret; the dataflows Function App reads it via a live `@Microsoft.KeyVault()` reference so a corrected value takes effect without a redeploy. Leave the Key Vault secret unset/empty to skip creating the alert. **Manual step after first setting a real value:** the Azure Monitor Action Group email receiver created from this address requires a one-time passcode verification, completed by opening the confirmation email sent to that address within 30 minutes of the action group being created or updated — this cannot be automated. |
