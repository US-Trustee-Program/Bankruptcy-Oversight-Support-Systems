...
This document tracks issues discovered during brand guidelines documentation that need team attention.
...
**Dead Code Locations**:
- `user-interface/src/App.scss` — `.App-header` uses `colors.$eagle` (`#2e2e2a`) but this is Create React App boilerplate, not the CAMS header. The whole `.App-header` block can likely be removed.
- `user-interface/src/styles/abstracts/_tables.scss` — Defines the following variables, none of which are used anywhere (except `$tableBorder` which is used in `App.scss`):
...
- `user-interface/src/styles/abstracts/_colors.scss` — Three variables defined but never used:
...
- The base `ComboBox.scss` has no color set on `.selection-label`, so all other instances use `#1b1b1b`
...
- Also includes `color: #333` on `span` elements inside the list items (another hardcoded color that would only apply if the class were used)
...
**Technical Details**:
- Code location: `user-interface/src/data-verification/trustee-verification/TrusteeMatchVerificationAccordion.scss` (line 139)
...
- Likely cause: `appEl` and `camsHeader` are queried via `document.querySelector` at component render time (lines 49-50), before the DOM may be fully ready. If either is `null`, the scroll event listener is never attached (line 146 guards on both values being truthy)
- The scroll listener is attached to `.App` div (not the browser window), which has `overflow-y: auto`

**Design Decision Needed**:
- Before fixing, **confirm with users whether a sticky header is actually desired**
- If users don't find value in it, consider removing the feature entirely rather than investing time in a fix
- If keeping it, move `appEl` and `camsHeader` queries inside the `useEffect` to ensure the DOM is ready when the listener is attached
...
- Defined variable: `$secondary-darker` (`#8b0a03`) in `_colors.scss`

**Next Steps**:
- Decide on the correct hover color
- Update `$secondary-darker` to match if needed, then replace the hardcoded value with the variable
...
- Hardcoded values: `#c6cace` (separator), `#565c65` (section separator), `#949494` (selected border), `#d0d0d0` (selected background), `#b0b0b0` (hover background)
...
**Technical Details**:
- Code location: `user-interface/src/lib/components/cams/RichTextEditor/RichTextEditor.scss`
- Hardcoded values: `#e9ecef` (toolbar background), `#71767a` (active button), `#f0f0f0` (hover), `#565c65` (muted controls), `#005ea6` (links — see issue #7)
...
`#5c5c5c`, `#565c65`, `#71767a`, `#949494`, `#b0b0b0`, `#c6c6c6`, `#c6cace`, `#d0d0d0`, `#e0e0e0`, `#e9ecef`, `#f0f0f0`

**Next Steps**:
- Define a standardized gray scale in `_colors.scss` (e.g., `$gray-dark`, `$gray-medium`, `$gray-light`, etc.) or map to USWDS gray tokens
...
2. **Only H3 has `line-height` defined** — `line-height: 1.5` is set on H3 only. All other headings inherit their line-height, which may vary by browser or USWDS base styles.

3. **H4 uses `margin-block-start/end` instead of `margin`** — All other headings use `margin: 0` or `margin` shorthand, but H4 uses the logical property `margin-block-start: 0; margin-block-end: 0`. Should be standardized.
...
- Note: Accent warm/cool colors ARE used in the `Tag` component (`bg-accent-cool`, `bg-accent-warm-dark`) — this is a separate thing from the button styles
...
- Mixin includes: `1px solid #ddd` cell borders, `3px solid black` row borders, `::before` pseudo-element labels via `data-cell` attribute
...
**Technical Details**:
- Code location: `user-interface/src/lib/components/uswds/modal/SubmitCancelButtonGroup.tsx` (line 74)
...
- The button gets its accessible name from its text content (works coincidentally) but has no formal label association
...
**Technical Details**:
- Code location: `user-interface/src/lib/components/uswds/Input.tsx`, `user-interface/src/lib/components/uswds/TextArea.tsx`

**Next Steps**:
- Add error element ID to `aria-describedby` when `errorMessage` is present (in addition to or replacing `aria-errormessage`)
...
**Next Steps**:
- Change `<span className="required-form-field" />` to `<span className="required-form-field">*</span>`
...
- Consider standardizing on appending a visually-hidden "(required)" span to labels, or including it in the label text
...
- Replace `<fieldset>` with a `<div role="radiogroup" aria-labelledby="...">` if the native fieldset styling is undesirable
...
- `user-interface/src/admin/bankruptcy-software/BankruptcySoftware.tsx` — single-column, no `data-cell`
- `user-interface/src/admin/bankruptcy-software/BankruptcySoftwareDetailTrustees.tsx` — single-column, no `data-cell`
- `user-interface/src/admin/bankruptcy-software/SoftwareBankTrustees.tsx` — single-column, no `data-cell`
...
**Next Steps**:
- Remove the `aria-label` prop and keep only `caption`, or remove `caption` and keep only `aria-label`
...
- `<MainContent>` wraps both `<Banner>` and `<header role="banner">`, making them children of `<main>`
...
- `role="navigation"` on `<nav>` — implicit; redundant in: `Header.tsx`, `CaseDetailNavigation.tsx`, `TrusteeDetailNavigation.tsx`, `AdminScreenNavigation.tsx`
...
**Technical Details**:
- Code location: `user-interface/src/lib/components/cams/SkipToMainContentLink/SkipToMainContentLink.tsx` (line 9)
...
**Next Steps**:
- Replace unlabeled `<section>` elements with `<div>` unless they should be navigable landmarks — in which case add `aria-label`
...
**Technical Details**:
- Code location: `user-interface/src/search-results/SearchResults.tsx` or `user-interface/src/search/SearchScreen.tsx`
...
- Hardcoded: `fill="#005EA2"` (primary blue) and `fill="#FEFFFF"` / `fill="white"` (near-white for arrow paths)

**Next Steps**:
- Replace hardcoded fill colors with `currentColor` and CSS classes, or
- If the two-color treatment is intentional (blue document + white arrows), document that explicitly and ensure the contrast meets WCAG AA requirements
