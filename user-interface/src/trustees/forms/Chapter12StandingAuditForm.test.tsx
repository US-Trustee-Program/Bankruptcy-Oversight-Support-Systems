...
import Chapter12StandingAuditForm, { buildAuditKeyDatesInput } from './Chapter12StandingAuditForm';
  ...
  return render(
    <BrowserRouter>
      <GlobalAlertContext.Provider value={mockGlobalAlertRef}>
        <Chapter12StandingAuditForm />
      </GlobalAlertContext.Provider>
    </BrowserRouter>,
  );
}

describe('Chapter12StandingAuditForm', () => {
  ...
  test('shows forbidden message when user lacks TrusteeAdmin role', async () => {
    ...
    expect(forbiddenAlert).toHaveTextContent(
      'You do not have permission to manage Trustee Audit Key Dates',
    );
  });
  ...
  test('shows loading spinner while fetching', () => {
    ...
    expect(screen.queryByTestId('edit-chapter12-standing-audit')).not.toBeInTheDocument();
  });
  ...
  test('shows empty inputs when API returns null', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('edit-chapter12-standing-audit')).toBeInTheDocument();
    });
  ...
  test('the status dropdown options read Closed/Not Closed (not Complete/Incomplete)', async () => {
    vi.spyOn(Api2, 'getUpcomingKeyDates').mockResolvedValue({ data: populatedDocument });

    renderComponent();

    await waitFor(() => {
      expect(screen.getByTestId('audit-completion-status-status')).toBeInTheDocument();
    });
    const statusSelect = screen.getByTestId('audit-completion-status-status');
    expect(statusSelect).toHaveTextContent('Closed');
    expect(statusSelect).toHaveTextContent('Not Closed');
    expect(statusSelect).not.toHaveTextContent('Complete');
    expect(statusSelect).not.toHaveTextContent('Incomplete');
  });
  ...
  test('shows inline error alert when key dates fail to load', async () => {
    ...
    expect(mockGlobalAlertRef.current.error).toHaveBeenCalledWith(
      'Failed to load Audit key dates: Network error',
    );
  });
  ...
  test('shows error alert when save fails and re-enables save button', async () => {
    ...
    await waitFor(() => {
      const saveButton = screen.getByTestId('button-save-chapter12-standing-audit');
      expect(saveButton).not.toBeDisabled();
      expect(saveButton).toHaveTextContent('Save');
    });
    expect(mockNavigate).not.toHaveBeenCalled();
    expect(mockGlobalAlertRef.current.error).toHaveBeenCalledWith(
      'Failed to save Audit key dates: Server error',
    );
  });
  ...
  test('cancel navigates back to the appointments page without saving', async () => {
    ...
    await userEvent.click(screen.getByTestId('button-cancel-chapter12-standing-audit'));
...
describe('buildAuditKeyDatesInput', () => {
  ...
  test('preserves every non-owned field from the original document and overrides only this card fields', () => {
    const result = buildAuditKeyDatesInput(
