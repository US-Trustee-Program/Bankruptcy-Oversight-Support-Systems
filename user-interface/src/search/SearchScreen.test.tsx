...
describe('search screen', () => {
  ...
  beforeEach(async () => {
    ...
    searchCasesSpy = vi
      ...
      .mockImplementation(() =>
        TestingUtilities.delayResolve<ResponseBody<SyncedCase[]>>(searchResponseBody),
      );
  });
  ...
  test('should render a list of cases by chapter number', async () => {
    ...
    // Default court search happens automatically
    await waitFor(() => {
      expect(loadingSpinner).not.toBeInTheDocument();
    });

    // Make additional search request...
    ...
    const courtDistrictClearButton = screen.getByTestId(
      'button-court-selections-search-clear-all-pills-button',
    );
    await userEvent.click(courtDistrictClearButton);
    await userEvent.click(searchButton);

    const defaultStateAlert = document.querySelector('#empty-state-alert');
  ...
  test('should render a list of cases by court division', async () => {
    ...
    await userEvent.click(expandButton);
    const list = screen.queryByTestId('court-selections-search-item-list-container');
    screen.debug(list!);
  ...
  test('should render a list of cases by case number', async () => {
    ...
    let defaultStateAlert = document.querySelector('#empty-state-alert');
    expect(defaultStateAlert).toBeInTheDocument();
    expect(defaultStateAlert).toBeVisible();
    ...
    await waitFor(() => {
      ...
      defaultStateAlert = document.querySelector('#empty-state-alert');
  ...
  test('should only search for full case number', async () => {
    ...
    await waitFor(() => {
      ...
      table = document.querySelector('.search-results table');
      expect(table).not.toBeInTheDocument();
      expect(screen.getByTestId('alert-message-empty-state-alert')).toBeInTheDocument();
    });
