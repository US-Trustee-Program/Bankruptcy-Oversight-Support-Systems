...
export default class Api {
  ...
  public static async delete(path: string, options?: ObjectKeyVal): Promise<ResponseBody | void> {
    try {
      ...
      const response = await httpDelete({ headers: this.headers, url: Api.host + pathStr });
  ...
  public static async get(path: string, options?: ObjectKeyVal): Promise<ResponseBody> {
    try {
      ...
      const apiOptions = this.getQueryStringsToPassThrough(window.location.search, options);
      const pathStr = Api.createPath(path, apiOptions);
      const response = await httpGet({ headers: this.headers, url: Api.host + pathStr });
      await this.executeAfterHooks(response);
  ...
  public static async patch(
    ...
    try {
      ...
      const apiOptions = this.getQueryStringsToPassThrough(window.location.search, options);
      const pathStr = Api.createPath(path, apiOptions);
      const response = await httpPatch({ body, headers: this.headers, url: Api.host + pathStr });
      await this.executeAfterHooks(response);
  ...
  public static async post(
    ...
    try {
      ...
      const response = await httpPost({ body, headers: this.headers, url: Api.host + pathStr });
      await this.executeAfterHooks(response);
  ...
  public static async put(
    ...
    try {
      ...
      const apiOptions = this.getQueryStringsToPassThrough(window.location.search, options);
      const pathStr = Api.createPath(path, apiOptions);
      const response = await httpPut({ body, headers: this.headers, url: Api.host + pathStr });
      await this.executeAfterHooks(response);
