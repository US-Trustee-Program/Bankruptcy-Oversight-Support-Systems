...
function setup(
  ...
  render(
    <PhoneEntryList
      phones={phones}
      onChange={onChange}
      typedPhonesEnabled={options.typedPhonesEnabled ?? true}
      errors={options.errors}
      legacyPhoneErrors={options.legacyPhoneErrors}
    />,
  );
...
describe('PhoneEntryList', () => {
  describe('typed phones enabled', () => {
    ...
    test('does not render the legacy single-phone inputs', () => {
      setup([{ type: 'direct', number: '555-111-2222' }]);

      expect(getLegacyPhoneInput()).not.toBeInTheDocument();
      expect(getLegacyExtensionInput()).not.toBeInTheDocument();
    });
  });

  describe('typed phones disabled (legacy mode)', () => {
    test('renders only the legacy phone and extension inputs, bound to the direct number', () => {
      const phones: TypedPhoneNumber[] = [
        { type: 'direct', number: '555-111-2222', extension: '99' },
        { type: 'home', number: '555-333-4444' },
      ];
      setup(phones, { typedPhonesEnabled: false });

      expect(getLegacyPhoneInput()).toHaveValue('555-111-2222');
      expect(getLegacyExtensionInput()).toHaveValue('99');
    });

    test('does not render the type select, add button, or remove button', () => {
      setup([{ type: 'direct', number: '555-111-2222' }], { typedPhonesEnabled: false });

      expect(getTypeSelect(0)).toBeNull();
      expect(screen.queryByRole('button', { name: /add another phone/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('button', { name: /remove phone entry/i })).not.toBeInTheDocument();
    });

    test('editing the legacy phone number updates only the direct-type entry', async () => {
      const onChange = vi.fn();
      const phones: TypedPhoneNumber[] = [
        { type: 'direct', number: '' },
        { type: 'home', number: '555-333-4444' },
      ];
      const { user } = setup(phones, { onChange, typedPhonesEnabled: false });

      await user.type(getLegacyPhoneInput(), '5551112222');
    ...
    test('editing the legacy extension updates only the direct-type entry', async () => {
      const onChange = vi.fn();
      const phones: TypedPhoneNumber[] = [{ type: 'direct', number: '555-111-2222' }];
      const { user } = setup(phones, { onChange, typedPhonesEnabled: false });

      await user.type(getLegacyExtensionInput(), '42');

      const lastCall = onChange.mock.calls.at(-1)![0] as TypedPhoneNumber[];
      expect(lastCall[0].extension).toMatch(/42/);
    });

    test('renders legacy phone and extension errors', () => {
      setup([{ type: 'direct', number: 'bad' }], {
        typedPhonesEnabled: false,
        legacyPhoneErrors: { phone: ['Invalid phone'], extension: ['Invalid extension'] },
      });
