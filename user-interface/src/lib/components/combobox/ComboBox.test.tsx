...
describe('ComboBox', () => {
  ...
  describe('Edge Cases', () => {
    ...
    test('getSelections() should return updated value immediately when called from onUpdateSelection callback', async () => {
      const ref = createRef<ComboBoxRef>();
      const options = getDefaultOptions(3);
      let selectionsFromCallback: ComboOption[] = [];

      const onUpdateSelection = vi.fn(() => {
        // Simulate reading from ref in callback (like PrivilegedIdentity does)
        selectionsFromCallback = ref.current?.getSelections() ?? [];
      });

      renderWithProps({ options, multiSelect: true, onUpdateSelection }, ref);

      await toggleDropdown(comboboxId);

      // Click first option to select it
      const firstOption = screen.getByTestId(`${comboboxId}-option-item-0`);
      ...
      // Verify that getSelections() returned the NEW selection, not stale data
      expect(selectionsFromCallback).toHaveLength(1);
      expect(selectionsFromCallback[0].value).toBe(options[0].value);

      // Click second option
      const secondOption = screen.getByTestId(`${comboboxId}-option-item-1`);
      ...
      // Should now have both selections
      expect(selectionsFromCallback).toHaveLength(2);
      expect(selectionsFromCallback.map((s) => s.value)).toEqual([
        options[0].value,
        options[1].value,
      ]);
    });
