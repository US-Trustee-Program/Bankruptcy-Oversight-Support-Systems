...
public class SqlQueryConstants {
  ...
  public static final String CASE_LIST_QUERY =
      "SELECT debtor.CURR_CASE_CHAPT, CONCAT(debtor.CASE_YEAR, '-', debtor.CASE_NUMBER) AS"
          ...
          + " 'STAFF1_PROF_NAME', proffestionaltypes1.PROF_TYPE_DESC AS 'STAFF1_PROF_TYPE_DESC',"
          + " RTRIM(professional2.PROF_FIRST_NAME) + ' ' + RTRIM(professional2.PROF_LAST_NAME) AS"
          + " 'STAFF2_PROF_NAME', proffestionaltypes2.PROF_TYPE_DESC AS 'STAFF2_PROF_TYPE_DESC',"
          ...
          + " = professional2.UST_PROF_CODE INNER JOIN [dbo].[CMMPT] proffestionaltypes1 ON"
          + " debtor.GROUP_DESIGNATOR = proffestionaltypes1.GROUP_DESIGNATOR AND"
          + " professional1.PROF_TYPE = proffestionaltypes1.PROF_TYPE INNER JOIN [dbo].[CMMPT]"
          + " proffestionaltypes2 ON debtor.GROUP_DESIGNATOR = proffestionaltypes2.GROUP_DESIGNATOR"
          + " AND professional2.PROF_TYPE = proffestionaltypes2.PROF_TYPE LEFT OUTER JOIN"
