...
/** Reads the most recently cached call's value for a comparison function out of a serialized
 * NormalizedMemo (see trustee-match-pipeline.ts) - memo[functionName] is an array of every
 * distinct-fingerprint call made against this candidate (see MemoEntry), not a single bare value,
 * since a comparison CAN be called more than once with different inputs. This reviewer only ever
 * displays a candidate's OWN latest fullNameSimilarity/tokenNameMatchRate, so the last entry is the
 * one that matters here. */
function latestMemoValue(memo: Record<string, MemoEntry[]>, functionName: string): unknown {
  const entries = memo[functionName];
...
function deriveCandidateFields(
  ...
  return {
    ...
    fullNameSimilarity,
    tokenNameMatchRate,
    acmsAddress: acmsAddressString(record.sourceRaw),
    acmsPhone: record.sourceRaw.legacy?.phone ?? '',
...
function formatAcmsRecord(record: ReviewRecord): string {
  return [
    `Full name: ${record.sourceRaw.fullName || '(blank)'}`,
    `Address: ${acmsAddressString(record.sourceRaw) || '(blank)'}`,
    `Phone: ${record.sourceRaw.legacy?.phone || '(blank)'}`,
  ].join('\n');
}
