...
# restores the original settings on exit (Ctrl+C or error). Faster than
# rebuilding the Docker image for iterative development — use start-services.sh
# for a fully containerized run instead.
#
# Usage:
#   ./start-funcapp.sh
#
# Prerequisites:
#   - MongoDB + Azurite running (podman run the two containers, or use
#     start-services.sh and ignore its dataflows container)
#   - local.settings.integration.json.template copied to
#     ../local.settings.integration.json
...
if [ ! -f "$INTEGRATION_SETTINGS" ]; then
  echo "ERROR: $INTEGRATION_SETTINGS not found — copy local.settings.integration.json.template first" >&2
  exit 1
fi
