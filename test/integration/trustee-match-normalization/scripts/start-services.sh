#!/bin/bash
# Start local infrastructure for trustee-match-normalization integration tests.
...
podman run -d \
  --pod "${POD_NAME}" \
  --name cams-tmn-mongodb \
  mongo:7.0 --bind_ip_all

echo "Waiting for MongoDB..."
for i in $(seq 1 30); do
  if bash -c '</dev/tcp/localhost/27018' 2>/dev/null; then
    echo "  MongoDB ready"
    break
  fi
...
echo ""
echo "Copy .env.template to .env.local (already pointed at port 27018)."
