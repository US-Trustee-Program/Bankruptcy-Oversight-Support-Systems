...
`processAppointments`'s decision tree) with eleven fixture cases.

## Scope

This harness tests the **existing (pre-Slice-5) matching algorithm** — CAMS-809's Slices 1-4
scoring/matching pipeline. The CAMS-809 Slice 5 fingerprint/variant memoization mechanism
(`trustee-variant.helpers.ts`, `TRUSTEE_VARIATION`) is already wired into `processAppointments`, but
since this harness seeds no `TRUSTEE_VARIATION` documents, every event here misses that bucket and
falls straight through to the algorithm below, exactly as it behaved before Slice 5. Folding
Slice-5-specific scenarios (fingerprint hit/miss, bucket+verify) into this same harness is a
...
| 2   | perfect-match-professional-id  | professional-id fast path resolves uniquely + active appointment in exact court/division/chapter       | Auto-linked, `trustee-match-verification` approved, `autoMatchCount++`                                                               |
...
| 7   | multiple-match-high-confidence | name ambiguous between 2 trustees; demographics clearly favor one                                      | Pending verification, `HIGH_CONFIDENCE_MATCH`, winner = the "real" trustee, no appointment created yet, `highConfidenceMatchCount++` |
...
Scenario 11 runs in two extra `processAppointments` calls after the main 10-scenario batch: first
resolving as `imperfect-match` (zero appointments, like #5), then a simulated human approval is
written directly to Mongo, then the same event is reprocessed to prove `upsertMatchVerification`'s
"already resolved, don't rewrite" guard.
...
| `seed-sql`    | Drop/recreate DXTR fixture rows for 11 scenarios (idempotent)     |
| `seed-cosmos` | Seed synced cases, trustees, appointments, professional ids       |
| `run`         | Full test: clean → seed → read DXTR → match/process (x3) → assert |
