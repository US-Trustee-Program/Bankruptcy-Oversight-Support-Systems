#!/bin/bash
# Start local infrastructure for trustee-match-scenarios integration tests.
...
podman run -d \
  --name cams-trustee-match-scenarios-mongodb \
  -p 27017:27017 \
  mongo:7.0 --bind_ip_all

echo "Starting SQL Edge..."
podman run -d \
  --name cams-trustee-match-scenarios-sqledge \
  -p 1433:1433 \
  -e ACCEPT_EULA=Y \
...
echo "  npm run trustee-match-scenarios -- seed-schema"
echo "  npm run trustee-match-scenarios -- seed-sql"
echo "  npm run trustee-match-scenarios -- seed-cosmos"
echo "  npm run trustee-match-scenarios -- run"
echo "  npm run trustee-match-scenarios -- clean"
