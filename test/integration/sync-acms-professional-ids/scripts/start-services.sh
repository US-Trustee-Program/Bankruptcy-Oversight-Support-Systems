#!/bin/bash
# Start local infrastructure for sync-acms-professional-ids integration tests.
...
IMAGE="localhost/integration_dataflows_acms_prof_ids:latest"
...
podman run -d \
  --pod "${POD_NAME}" \
  --name cams-mongodb-sync-acms-professional-ids \
  mongo:7.0 --bind_ip_all

echo "Starting SQL Edge..."
podman run -d \
  --pod "${POD_NAME}" \
  --name cams-sqledge-sync-acms-professional-ids \
  -e ACCEPT_EULA=Y \
...
podman run -d \
  --pod "${POD_NAME}" \
  --name cams-dataflows-sync-acms-professional-ids \
  "${IMAGE}"
