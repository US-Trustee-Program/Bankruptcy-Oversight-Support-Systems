#!/bin/bash
# Start local infrastructure for backfill-unassigned-on integration tests.
...
podman run -d \
  --pod "${POD_NAME}" \
  --name cams-backfill-unassigned-on-mongodb \
...
echo ""
echo "Create .env.local with MONGO_CONNECTION_STRING and COSMOS_DATABASE_NAME set."
