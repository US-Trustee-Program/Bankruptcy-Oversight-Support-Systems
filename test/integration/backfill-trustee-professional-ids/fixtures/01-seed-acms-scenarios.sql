-- ACMS fixture rows for the 6 backfill-trustee-professional-ids scenarios (see README.md's
-- coverage table). Drop-and-recreate by reserved key blocks so this file is fully idempotent.
--
-- Reserved namespace:
--   GROUP_DESIGNATOR = 'BT'            (all CMMPR/CMMAP rows in this harness)
--   UST_PROF_CODE / PROF_CODE          97001-97006 (one per scenario)
--   CASE_DIV                           601-604 (scenarios 1/2/5), 701-711 (scenario 3)
--
...
-- ---------------------------------------------------------------------------
-- CMMDO — division/office map. Scenario 3 needs 11 distinct courts to reach a genuine
-- 9-of-10 overlap-coefficient fraction (90); scenarios 1/2/5 need one/two courts each.
-- ---------------------------------------------------------------------------

INSERT INTO dbo.CMMDO (CASE_DIV, COURT_ID, DELETE_CODE) VALUES
  (601, 'BT01', ' '),  -- scenario 1
  (602, 'BT02', ' '),  -- scenario 2
  (603, 'BT03', ' '),  -- scenario 5, appointment A
  (604, 'BT04', ' '),  -- scenario 5, appointment B
  (701, 'S301', ' '),  -- scenario 3, court 1 of 10 (shared by ACMS + winner + runner-up)
  (702, 'S302', ' '),  -- scenario 3, court 2 of 10
  (703, 'S303', ' '),  -- scenario 3, court 3 of 10
  (704, 'S304', ' '),  -- scenario 3, court 4 of 10
  (705, 'S305', ' '),  -- scenario 3, court 5 of 10
  (706, 'S306', ' '),  -- scenario 3, court 6 of 10
  (707, 'S307', ' '),  -- scenario 3, court 7 of 10
  (708, 'S308', ' '),  -- scenario 3, court 8 of 10
  (709, 'S309', ' '),  -- scenario 3, court 9 of 10
  (710, 'S310', ' '),  -- scenario 3, court 10 of 10 (ACMS + winner only, NOT in runner-up's set)
  (711, 'S311', ' ');  -- scenario 3, runner-up's 10th court (NOT in ACMS's set)
GO
...
VALUES
  -- Scenario 1: Tier-1-only. Exact name+state match; the corresponding CAMS trustee has no
  -- phoneticTokens seeded, so Tier 2 (which pre-filters on phoneticTokens) misses it entirely.
  (' ', 'BT', 97001, 'Robert', 'Ashworth-Quintela', NULL,
   '100 Ashworth Ln', NULL, 'Springfield', 'IL', 62701, 2175550001,
   'TR', GETDATE()),

  -- Scenario 2: Tier-2-only. PROF_STATE is stale/wrong ('ZZ') so Tier 1's exact name+state query
  -- fails on state; Tier 2's name-only phonetic search still finds the trustee (real state 'NM').
  (' ', 'BT', 97002, 'Jonathan', 'Villareal', NULL,
   '200 Villareal Ave', NULL, 'Santa Fe', 'ZZ', 87501, 5055550002,
   'TR', GETDATE()),

  -- Scenario 3: multi-candidate gap-check. Two CAMS trustees share this exact name+state; both
  ...
  -- Scenario 4: lone candidate below threshold. Name matches exactly (so a candidate is found
  -- via Tier 1), but address/phone/appointment-history corroboration is absent or mismatched,
  ...
  -- Scenario 5 (THE most important regression case): every CMMAP appointment row for this
  ...
  -- Scenario 6: already-mapped idempotency. A trustee-professional-ids mapping for this ACMS
...
-- except scenario 5's two rows, which are deliberately closed pre-2018.
-- ---------------------------------------------------------------------------
...
VALUES
  -- Scenario 1: one open case, chapter 7.
  (601, 24, 10001, 0, 0, ' ', 20240115, '7'),

  -- Scenario 2: one open case, chapter 11.
  (602, 24, 10002, 0, 0, ' ', 20240201, '11'),

  -- Scenario 5: TWO closed, pre-2018 cases -- the regression-proof rows. Both closed dates are
  ...
  -- Scenario 6: one open case, chapter 7 (own case row -- distinct from scenario 1's, which
  -- shares CASE_DIV 601 but must not share the same (CASE_DIV, CASE_YEAR, CASE_NUMBER) key).
  (601, 24, 10007, 0, 0, ' ', 20240115, '7'),

  -- Scenario 3: ten open cases across ten distinct courts (S301..S310), all chapter 7.
...
-- Scenario 4 deliberately has ZERO CMMAP rows (no appointment history at all).
-- Scenario 6 gets one row for realism, though it's never fetched (its professional id is
...
VALUES
  -- Scenario 1: BT-97001 appointed in court BT01 / chapter 7.
  (1, 601, 24, 10001, 'BT', 97001, 20240115, 0, ' ', 'Y', 'TR'),

  -- Scenario 2: BT-97002 appointed in court BT02 / chapter 11.
  (1, 602, 24, 10002, 'BT', 97002, 20240201, 0, ' ', 'Y', 'TR'),

  -- Scenario 5: BT-97005 appointed in BOTH closed pre-2018 cases (courts BT03/BT04).
  (1, 603, 15, 10005, 'BT', 97005, 20150101, 20150615, ' ', 'Y', 'TR'),
  (1, 604, 16, 10006, 'BT', 97005, 20160201, 20160910, ' ', 'Y', 'TR'),

  -- Scenario 6: BT-97006 appointed once, for realism only (never fetched — see comment above).
  (1, 601, 24, 10007, 'BT', 97006, 20240115, 0, ' ', 'Y', 'TR'),

  -- Scenario 3: BT-97003 appointed once in EACH of the ten S301..S310 courts (10 CMMAP rows,
