...
See `/tmp/cams-816-collab/TRUSTEE-ACMS-BACKFILL_CONVERGED_DESIGN.md`'s "Implementation-scope note on
integration testing" section for the full rationale: Brian's standing preference is to lean on real
integration-test harnesses for database-touching code specifically because mocks can pass while the
real query is subtly wrong ("mocks lie"). This applies fully to the **new** code this epic
introduces — the widened `getAllTrusteeProfessionalRecords`, the new batched
...
| `seed-sql`    | Drop/recreate ACMS fixture rows for the 6 scenarios (idempotent) — `fixtures/01-seed-acms-scenarios.sql`                |
...
- **`CMMAP`/`CMMDB`** — reused verbatim from
  ...
  `DELETE_CODE` on `CMMDB`). **`CMMKE` is deliberately NOT created** — the new query does not join
  it (that's the point of the new query; see the converged design doc's "Appointment-context query
  shape" section).
...
`fixtures/01-seed-acms-scenarios.sql` seeds the ACMS-side (SQL) half of all 6 scenarios; the harness
script's `seedCosmos()` seeds the matching CAMS-side (Mongo) half. Reserved namespace:
`GROUP_DESIGNATOR = 'BT'`, `UST_PROF_CODE`/`PROF_CODE` block `97001`-`97006`, `CASE_DIV` blocks
`601`-`604` and `701`-`711`, `trusteeId` prefix `bkotp-*`.

| #   | Scenario                                                                            | ACMS id    | Setup                                                                                                                                                                                                                                                                                                       | Expected outcome                                                                                                                                                                                                                                                                                                                                                                       |
| --- | ----------------------------------------------------------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Tier-1-only match**                                                               | `BT-97001` | Exact name+state match against `bkotp-s1-correct`; that trustee's Mongo doc deliberately has **no `phoneticTokens`** seeded (simulating a trustee never run through the phonetic-token backfill)                                                                                                            | Tier 1 (`findTrusteeByNameAndState`) finds it; Tier 2 (`searchTrusteesByNameScored`) misses it entirely because its own `$match` pre-filter requires `phoneticTokens` — asserted directly by calling Tier 2 alone and confirming absence. Full corroboration (address/phone/district/chapter) → score 100 → **matched**                                                                |
| 2   | **Tier-2-only match**                                                               | `BT-97002` | ACMS `PROF_STATE` is stale/wrong (`'ZZ'`); the matching CAMS trustee `bkotp-s2-correct` has the real state (`'NM'`). Name is otherwise identical                                                                                                                                                            | Tier 1 fails (state gate); Tier 2 (no state filter) finds it — asserted directly by calling Tier 1 alone and confirming `null`. Name/phone/district/chapter all score 100, but ACMS's own deliberately-wrong state also degrades the address score to 60 (zip-match tier, since state itself doesn't match) — total ≈ 97.89, still comfortably clearing the 90 threshold → **matched** |
| 3   | **Multi-candidate gap-check**                                                       | `BT-97003` | Two CAMS trustees (`bkotp-s3-winner`, `bkotp-s3-runnerup`) share the identical name/state/address/phone. Winner's district set matches ACMS's 10-court set exactly (district=100); runner-up's district set shares 9 of those 10 courts plus one different court (district=90, via the overlap coefficient) | Winner scores 100, runner-up scores ~96.84 — both individually clear the 90 auto-match threshold, but the gap between them (~3.16) is `< ACMS_FUZZY_MATCH_MIN_GAP` (5) → **neither is matched** (the accept-rule shape fix in action)                                                                                                                                                  |
| 4   | **Lone candidate below threshold**                                                  | `BT-97004` | `bkotp-s4-weak` is found via Tier 1 (exact name match) but has a mismatched address, no phone on file, and zero appointment history on either side (district/chapter both `null`, not `0`)                                                                                                                  | Total score ≈ 83.3 `< 90` → **permanently unmatched**; asserted by confirming absence of any `trustee-professional-ids` document for `BT-97004` — no JSONL, no review file, just absence                                                                                                                                                                                               |
...
| 6   | **Already-mapped idempotency**                                                      | `BT-97006` | A `trustee-professional-ids` mapping (`bkotp-s6-existing` ↔ `BT-97006`) is pre-seeded by the harness before `processAcmsProfessionalRecordsPage` runs                                                                                                                                                       | The record is skipped via `findByAcmsProfessionalId` and counted as `alreadyMapped` — never scored, never re-written. Re-running the full page a second time leaves exactly one mapping document, `matched` for this id stays `0`, `alreadyMapped` for it stays `1` — a safe no-op                                                                                                     |

`run` also re-runs `processAcmsProfessionalRecordsPage` a second time over the same record set after
the first pass completes (Stage 5), independent of scenario 6's own pre-seeded mapping, to confirm
scenarios 1/2/5's newly-created mappings are themselves idempotent on a second pass (`alreadyMapped`
picks them up, `matched` does not double-count them, and each still has exactly one mapping
document).

## Assertion summary

Beyond the six scenario outcomes in the table above, `run` also asserts:
