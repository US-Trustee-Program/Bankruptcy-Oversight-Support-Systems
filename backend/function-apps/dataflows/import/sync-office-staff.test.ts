...
describe('sync-office-staff timerTrigger', () => {
  ...
  beforeEach(async () => {
    ...
    invocationContext = {
      ...
      functionName: 'sync-office-staff-timerTrigger',
      extraOutputs: {
        set: vi.fn(),
        get: vi.fn(),
      },
  ...
  test('completes trace with success true when controller succeeds', async () => {
    vi.spyOn(OfficesController.prototype, 'handleTimer').mockResolvedValue({
      success: 5,
      fail: 0,
    });
  ...
  test('handles 429 gracefully: does not throw, emits rate-limited trace, skips toAzureError', async () => {
    ...
    vi.spyOn(OfficesController.prototype, 'handleTimer').mockRejectedValue(rateLimitError);
  ...
  test('handles non-429 errors: calls toAzureError and emits failure trace', async () => {
    ...
    vi.spyOn(OfficesController.prototype, 'handleTimer').mockRejectedValue(genericError);
