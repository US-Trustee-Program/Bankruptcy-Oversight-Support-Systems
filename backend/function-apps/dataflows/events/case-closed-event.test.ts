...
const makeInvocationContext = (): InvocationContext =>
  ({
    ...
    functionName: 'case-closed-event-handler',
    extraOutputs: new Map(),
    log: vi.fn(),
  }) as unknown as InvocationContext;

const makeCaseClosedMessage = (overrides?: Partial<CaseAssignment & { retryCount?: number }>) => ({
...
describe('case-closed-event handler', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  test('should process event and emit success telemetry', async () => {
    const { handler } = await import('./case-closed-event');
    const message = makeCaseClosedMessage();
    const invocationContext = makeInvocationContext();

    vi.spyOn(OfficeAssigneesUseCase, 'handleCaseClosedEvent').mockResolvedValue(undefined);
    ...
    expect(OfficeAssigneesUseCase.handleCaseClosedEvent).toHaveBeenCalledWith(
      expect.anything(),
      message,
    );
    expect(telemetrySpy).toHaveBeenCalledWith(
      expect.anything(),
      expect.anything(),
      expect.stringContaining('CASE-CLOSED-EVENT'),
      'handler',
      expect.anything(),
  ...
  test('should re-enqueue with backoff on 429 error', async () => {
    const { handler } = await import('./case-closed-event');
    const message = makeCaseClosedMessage({ retryCount: 0 });
    const invocationContext = makeInvocationContext();

    const tooManyError = new TooManyRequestsError('CASE-CLOSED-EVENT');
    vi.spyOn(OfficeAssigneesUseCase, 'handleCaseClosedEvent').mockRejectedValue(tooManyError);
    vi.spyOn(ApplicationContextCreator, 'getApplicationContext').mockResolvedValue(
      await createMockApplicationContext(),
    );
  ...
  test('should route to DLQ and emit telemetry when retry limit exhausted', async () => {
    const { handler } = await import('./case-closed-event');
    const message = makeCaseClosedMessage({ retryCount: 10 });
    const invocationContext = makeInvocationContext();
  ...
  test('should route to DLQ and throw on non-429 error', async () => {
    ...
    const error = new CamsError('CASE-CLOSED-EVENT', { message: 'Database connection failed' });
    vi.spyOn(OfficeAssigneesUseCase, 'handleCaseClosedEvent').mockRejectedValue(error);
    vi.spyOn(ApplicationContextCreator, 'getApplicationContext').mockResolvedValue(
      await createMockApplicationContext(),
    );
