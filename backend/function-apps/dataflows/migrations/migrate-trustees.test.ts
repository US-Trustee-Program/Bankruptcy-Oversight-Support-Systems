...
describe('migrate-trustees', () => {
  ...
  describe('handleStart — flushQueues', () => {
    // flushQueues is a failure/DLQ inspection tool: it drains exactly two
    // queues — DLQ and failed-appointments. The transient START and PAGE work
    // queues are deliberately excluded.
    test('dumps the failure queues to blobs when every queue exists', async () => {
    ...
    test('drains queued messages, deletes them, and writes decoded JSONL to blob storage', async () => {
      ...
      // Flush order: DLQ (empty); FAILED_APPOINTMENTS (two messages across two pages).
    ...
    test('does not flush the START or PAGE work queues', async () => {
      ...
      // transient start/page work queues. We drain 2 queues, neither of which is
      ...
      expect(flushedQueueNames).toHaveLength(2);
