...
| 5 | Missed downstream events are acceptable if compensated — they are replayable via error documents in Cosmos. | On queue failure, a `STAFF_ASSIGNMENT_DOWNSTREAM_SYNC_ERROR` or `TRUSTEE_APPOINTMENT_DOWNSTREAM_SYNC_ERROR` document is written to Cosmos. If Cosmos is also unavailable, the failure is logged but does not abort remaining assignments. |
