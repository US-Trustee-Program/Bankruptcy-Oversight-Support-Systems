...
export class CasesMongoRepository extends BaseMongoRepository implements CasesRepository {
  ...
  async getConsolidation(caseId: string): Promise<Array<ConsolidationFrom | ConsolidationTo>> {
    const doc = using<ConsolidationFrom | ConsolidationTo>();
    try {
      const query = and(doc('documentType').regex('^CONSOLIDATION_'), doc('caseId').equals(caseId));
      const adapter = this.getAdapter<ConsolidationFrom | ConsolidationTo>();
      return await adapter.find(query);
    } catch (originalError) {
