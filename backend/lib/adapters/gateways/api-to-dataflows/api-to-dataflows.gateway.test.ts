...
describe('ApiToDataflowsGatewayImpl', () => {
  ...
  describe('queueCaseReload', () => {
    test('should wrap single case reload event in an array when queuing to page queue', async () => {
      ...
      // Verify that extraOutputs.set receives an array containing the event
      // This ensures the sync-cases handlePage function receives CaseSyncEvent[]
  ...
  describe('queueCaseAssignmentEvent', () => {
    test('should queue case assignment event as single object', async () => {
      const gateway = new ApiToDataflowsGatewayImpl(mockContext);
      ...
      // Case assignment events are queued as single objects, not arrays
