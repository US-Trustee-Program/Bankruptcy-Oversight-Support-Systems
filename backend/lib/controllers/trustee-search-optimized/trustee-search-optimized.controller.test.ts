...
import { TrusteeSearchOptimizedController } from './trustee-search-optimized.controller';
import { TrusteeSearchOptimizedUseCase } from '../../use-cases/trustees/trustee-search-optimized.use-case';
import { createMockApplicationContext } from '../../testing/testing-utilities';
import { ApplicationContext } from '../../adapters/types/basic';
import { CamsRole } from '@common/cams/roles';
import { TrusteeSearchResult } from '@common/cams/trustee-search';

describe('TrusteeSearchOptimizedController', () => {
  ...
  test('should return search results for a valid name query', async () => {
    vi.spyOn(TrusteeSearchOptimizedUseCase.prototype, 'searchTrustees').mockResolvedValue(
      mockSearchResults,
    );

    const controller = new TrusteeSearchOptimizedController();
    const response = await controller.handleRequest(context);

    expect(TrusteeSearchOptimizedUseCase.prototype.searchTrustees).toHaveBeenCalledWith(
      context,
      'smith',
      undefined,
    );
  ...
  test('should propagate errors from use case as CamsError', async () => {
    vi.spyOn(TrusteeSearchOptimizedUseCase.prototype, 'searchTrustees').mockRejectedValue(
