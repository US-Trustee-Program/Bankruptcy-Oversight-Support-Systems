...
describe('TrusteeCasesUseCase', () => {
  ...
  test('filters appointments by appointedDateTo in memory; excluded appointments return no cases', async () => {
    const context = await createMockApplicationContext();
    const appointments = [
      {
        id: 'appt-1',
    ...
    // appt-2 has appointedDate 2025-01-01 which is after appointedDateTo, so only appt-1 passed to searchCases
    expect(searchSpy).toHaveBeenCalledWith(expect.objectContaining({ caseIds: ['081-24-00001'] }));
    expect(searchSpy).not.toHaveBeenCalledWith(
      expect.objectContaining({ appointedDateTo: expect.anything() }),
    );
    expect(result.data).toHaveLength(1);
    expect(result.data[0].caseId).toBe('081-24-00001');
  });

  test('filters appointments by appointedDateFrom in memory; earlier appointments excluded', async () => {
    const context = await createMockApplicationContext();
    const appointments = [
      {
        id: 'appt-1',
    ...
    const result = await useCase.getCasesForTrustee(context, 'trustee-abc', {
      limit: 25,
      offset: 0,
      appointedDateFrom: '2024-01-01',
    });

    // appt-1 has appointedDate 2023-01-01 which is before appointedDateFrom, so only appt-2 passed
    expect(searchSpy).toHaveBeenCalledWith(expect.objectContaining({ caseIds: ['081-24-00002'] }));
    expect(result.data).toHaveLength(1);
    expect(result.data[0].caseId).toBe('081-24-00002');
  });

  test('returns empty when all appointments are outside the appointed date range', async () => {
    const context = await createMockApplicationContext();
    const appointments = [
      {
        id: 'appt-1',
