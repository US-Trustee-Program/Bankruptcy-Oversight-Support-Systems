...
import { TrusteeSearchOptimizedUseCase } from './trustee-search-optimized.use-case';
...
describe('TrusteeSearchOptimizedUseCase', () => {
  ...
  function setupRepositories(options: {
    ...
    const searchByNameScoredOptimizedMock = vi.fn().mockResolvedValue(scoredResults);

    vi.spyOn(factory, 'getTrusteesRepository').mockReturnValue(
      Object.assign(new MockMongoRepository(), {
        searchTrusteesByNameScoredOptimized: searchByNameScoredOptimizedMock,
      }),
    );
    ...
    return { searchByNameScoredOptimizedMock, appointmentsMock };
  }
  ...
  test('should return trustees from scored results', async () => {
    ...
    const useCase = new TrusteeSearchOptimizedUseCase();
  ...
  test('should fire TrusteeManualSearchOptimizedPerformed telemetry on success', async () => {
    const appointments = new Map<string, Partial<TrusteeAppointment>[]>();
    appointments.set('trustee-001', mockAppointments1);
    ...
    expect(completeSpy).toHaveBeenCalledWith(
      expect.any(Object),
      'TrusteeManualSearchOptimizedPerformed',
  ...
  test('should fire TrusteeManualSearchOptimizedPerformed with success false on error and propagate', async () => {
    vi.spyOn(factory, 'getTrusteesRepository').mockReturnValue(
      Object.assign(new MockMongoRepository(), {
        searchTrusteesByNameScoredOptimized: vi.fn().mockRejectedValue(new Error('DB failure')),
      }),
    );
