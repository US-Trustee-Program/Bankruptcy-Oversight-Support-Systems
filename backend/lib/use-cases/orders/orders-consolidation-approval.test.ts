...
describe('Orders use case', () => {
  ...
  test('should approve a split consolidation order', async () => {
    ...
    originalConsolidation.memberCases.push(
    ...
    const approval: ConsolidationOrderActionApproval = {
      ...
      approvedCases: [
        originalConsolidation.memberCases[0].caseId,
        originalConsolidation.memberCases[1].caseId,
      ],
    ...
    const approvedConsolidation = {
      ...originalConsolidation,
      memberCases: [originalConsolidation.memberCases[0], originalConsolidation.memberCases[1]],
      leadCase: leadCaseSummary,
    ...
    const newPendingConsolidation = {
      ...originalConsolidation,
      memberCases: [originalConsolidation.memberCases[2]],
    };
    ...
    const memberCaseSummaries = approvedConsolidation.memberCases.map((bCase) =>
      getCaseSummaryFromConsolidationOrderCase(bCase),
    );
    const leadCaseAfter: ConsolidationOrderSummary = {
      status: 'approved',
      memberCases: [memberCaseSummaries[0], memberCaseSummaries[1]],
    };
    ...
    const memberCaseHistory: Partial<CaseHistory> = {
      documentType: 'AUDIT_CONSOLIDATION',
      caseId: originalConsolidation.memberCases[0].caseId,
      before,
      after,
    };
    ...
    vi.spyOn(MockMongoRepository.prototype, 'createConsolidationFrom').mockResolvedValue(
      MockData.getConsolidationFrom({ override: { otherCase: memberCaseSummaries[0] } }),
    );
  ...
  test('should approve a consolidation order', async () => {
    ...
    const memberCaseSummaries = newConsolidation.memberCases.map((bCase) =>
      getCaseSummaryFromConsolidationOrderCase(bCase),
    );

    const leadCaseAfter: ConsolidationOrderSummary = {
      status: 'approved',
      memberCases: memberCaseSummaries,
    };
    ...
    const actual = await useCase.approveConsolidation(mockContext, approval);
    expect(mockGetConsolidation).toHaveBeenCalledTimes(pendingConsolidation.memberCases.length + 1);
    ...
    expect(mockCreateHistory.mock.calls[2][0]).toEqual(expect.objectContaining(leadCaseHistory));
    expect(mockGetHistory).toHaveBeenCalledTimes(pendingConsolidation.memberCases.length + 1);
    expect(mockCreateAssignment).toHaveBeenCalledTimes(pendingConsolidation.memberCases.length);
