...
describe('Export and Load Case Tests', () => {
  ...
  describe('exportAndLoadCase', () => {
    ...
    test('should call matchTrusteeByName for TRUSTEE_APPOINTMENT events and set trusteeId', async () => {
      const trustee = MockData.getTrustee();
      const mockCaseDetails = MockData.getCaseDetail({
        override: { trustee: MockData.getLegacyTrustee({ name: trustee.name }) },
      });
      const events: CaseSyncEvent[] = [{ type: 'TRUSTEE_APPOINTMENT', caseId: '081-20-10001' }];
      ...
      expect(syncedCase.trusteeId).toBe(trustee.trusteeId);
    });

    test('should NOT call matchTrusteeByName for CASE_CHANGED events', async () => {
      const mockCaseDetails = MockData.getCaseDetail({
        override: { trustee: MockData.getLegacyTrustee({ name: 'Some Trustee' }) },
      });
      const events: CaseSyncEvent[] = [{ type: 'CASE_CHANGED', caseId: '081-20-10001' }];
      ...
      const syncedCase = syncSpy.mock.calls[0][0];
      expect(syncedCase.trusteeId).toBeUndefined();
    });

    test('should set error on event when trustee matching fails', async () => {
      const mockCaseDetails = MockData.getCaseDetail({
        override: { trustee: MockData.getLegacyTrustee({ name: 'Unknown Trustee' }) },
      });
      const events: CaseSyncEvent[] = [{ type: 'TRUSTEE_APPOINTMENT', caseId: '081-20-10001' }];
      ...
      expect(actual[0].error.message).toContain('No CAMS trustee found matching name');
    });

    test('should skip trustee matching for TRUSTEE_APPOINTMENT when case has no trustee name', async () => {
      const mockCaseDetails = MockData.getCaseDetail({ override: { trustee: undefined } });
      const events: CaseSyncEvent[] = [{ type: 'TRUSTEE_APPOINTMENT', caseId: '081-20-10001' }];
