...
import BackfillTransferOrderTaskDateUseCase from './backfill-transfer-order-task-date';
...
function makeOrder(override: Partial<TransferOrder> = {}): BackfillTransferOrder {
  const order = MockData.getTransferOrder({ override });
  return { ...order, _id: order.id };
}

describe('BackfillTransferOrderTaskDateUseCase', () => {
  ...
  describe('getPageNeedingBackfill', () => {
    test('should return a page of transfer orders missing taskDate', async () => {
      const order = makeOrder({ orderDate: '2025-01-10T00:00:00.000Z' });
      ...
      const result = await BackfillTransferOrderTaskDateUseCase.getPageNeedingBackfill(
        context,
        null,
        100,
      );
  ...
  describe('backfillTaskDates', () => {
    test('should set taskDate from orderDate for each order', async () => {
      const order = makeOrder({ orderDate: '2025-01-10T00:00:00.000Z' });
      const updateSpy = vi
        .spyOn(MockMongoRepository.prototype, 'updateTransferOrderTaskDate')
        .mockResolvedValue();
    ...
    test('should record failure when update throws, and continue processing', async () => {
      const order1 = makeOrder({ orderDate: '2025-01-10T00:00:00.000Z' });
      const order2 = makeOrder({ orderDate: '2025-01-15T00:00:00.000Z' });

      vi.spyOn(MockMongoRepository.prototype, 'updateTransferOrderTaskDate')
        .mockRejectedValueOnce(new Error('write failed'))
        .mockResolvedValueOnce(undefined);

      const result = await BackfillTransferOrderTaskDateUseCase.backfillTaskDates(context, [
        order1,
        order2,
      ]);
  ...
  describe('processBackfillPage', () => {
    test('should return ok with processedCount and nextCursor when more pages remain', async () => {
      const order1 = makeOrder({ orderDate: '2025-01-10T00:00:00.000Z' });
      const order2 = makeOrder({ orderDate: '2025-01-15T00:00:00.000Z' });
      ...
      vi.spyOn(MockMongoRepository.prototype, 'updateTransferOrderTaskDate').mockResolvedValue();
