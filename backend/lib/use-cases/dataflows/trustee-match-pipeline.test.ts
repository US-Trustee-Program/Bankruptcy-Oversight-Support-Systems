...
describe('addDisqualifier', () => {
  ...
  test('does not affect a different candidate in the same state', () => {
    const state = createInitialState(makeDxtrTrustee());
    const disqualified = addCandidate(state, projectTrustee(makeTrustee({ trusteeId: 't1' })));
    const untouched = addCandidate(state, projectTrustee(makeTrustee({ trusteeId: 't2' })));
