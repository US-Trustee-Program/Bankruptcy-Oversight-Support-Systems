...
describe('Orders use case', () => {
  ...
  // TODO:
  // The following 2 tests, if either one of them is enabled, breaks.
  // Trying to track down this error:  Cannot read properties of undefined (reading 'dateFiled')
  // I put console logs in cases.local.gateway->getCases() but they are not being printed
  // so I'm not sure that is the method that is being called.
  test.skip('should approve a consolidation order', async () => {
    const pendingConsolidation = MockData.getConsolidationOrder({
      override: {
        status: 'approved',
      },
  ...
  test.skip('should approve a split consolidation order', async () => {
