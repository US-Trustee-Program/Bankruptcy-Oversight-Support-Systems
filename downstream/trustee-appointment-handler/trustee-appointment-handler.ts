...
import { CmmapCamsRow } from '../shared/cmmap-cams-row';

export type { TrusteeAppointmentDownstreamEvent };
export type { CmmapCamsRow };
...
async function upsertTrusteeAppointment(row: CmmapCamsRow): Promise<void> {
...
async function trusteeAppointmentHandler(
  ...
  try {
    ...
    const stagingRow = transformTrusteeToStagingRow(appointmentEvent);
    await upsertTrusteeAppointment(stagingRow);
